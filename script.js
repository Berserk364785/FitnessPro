...
async function updateLB(){
  ...
  const statusLine=isCloud
    ...
    :CLOUD_ENABLED
      ?`<div class="lb-status offline">⚠️ Не удалось загрузить онлайн-рейтинг</div>`
...
async function publishToCloud(silent=true){
  ...
  const ok=await cloudPublishScore({name:userName,avatar,avatarIsPhoto,xp,lvl,maxStreak});
...
function speak(txt,urgency=''){
  ...
  lastVoiceTs=now;
  if(urgency==='!'){speechSynthesis.cancel();speakQueue=[];}
...
function pumpSpeakQueue(){
  ...
  u.pitch=voicePitch;
  u.onend=u.onerror=()=>{speakBusy=false;pumpSpeakQueue();};
...
window.onload=()=>{
  ...
  q('testVoiceBtn')?.addEventListener('click',()=>speak('Привет! Я ваш голосовой тренер. Готовы тренироваться?','!'));
