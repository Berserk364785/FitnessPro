...
let pose=null,cam=null,isRunning=false,isPaused=false;
let lastVoiceTs=0,angHistory=[],lastSmooth=null;
const DEAD=2;
...
let avatar='🏆',avatarIsPhoto=false;
...
// ============================================================
const EX={
  pushup:{name:'Отжимания',dn:80,up:160,ang:'elbow',cal:.5,xp:10,emoji:'💪',meta:'Грудь, трицепс'},
  squat:{name:'Приседания',dn:100,up:170,ang:'knee',cal:.7,xp:12,emoji:'🦵',meta:'Квадрицепс, ягодицы'},
  plank:{name:'Планка',dn:160,up:180,ang:'sha',isPlank:true,calS:.15,xpS:2,emoji:'⏱️',meta:'Кор, пресс'},
  situp:{name:'Пресс',dn:45,up:90,ang:'trunk',cal:.4,xp:8,emoji:'🔺',meta:'Пресс'},
  lunge:{name:'Выпады',dn:90,up:170,ang:'knee',cal:.6,xp:10,emoji:'🏃',meta:'Ноги, баланс'},
  burpee:{name:'Бёрпи',dn:80,up:160,ang:'elbow',cal:1.0,xp:20,emoji:'🤸',meta:'Всё тело'},
  pullup:{name:'Подтягивания',dn:140,up:30,ang:'elbow',cal:.8,xp:15,emoji:'🧗',meta:'Спина, бицепс'}
};
...
function load(){
  ...
  xp=d.xp||0;lvl=d.lvl||1;streak=d.streak||0;maxStreak=d.maxStreak||0;
...
// ============================================================
function addXP(a){
  xp+=a;let need=lvl*100,leveled=false;
  while(xp>=need){xp-=need;lvl++;need=lvl*100;leveled=true;speak(`Уровень ${lvl}!`,'!');bLvl();confetti(2500);toast(`🎉 Уровень ${lvl}!`);}
  updateLvlUI();if(leveled)save();
}
...
function addRep(){
  ...
  addXP(e.xp);addRepCal();updStreak(true);checkQuest();checkChallenge();
...
function addPlankT(dt){
  ...
  addXP(e.xpS*dt);addPlankCal(dt);updStreak(true);checkQuest();checkChallenge();updProgress();checkAch();
}
function resetReps(){
  repCount=0;q('bigNum').textContent='0';isDown=false;plankTime=0;plankActive=false;goalAchieved=false;
...
//  ANGLES / POSE
// ============================================================
function smooth(a){angHistory.push(a);if(angHistory.length>10)angHistory.shift();return angHistory.reduce((s,v)=>s+v,0)/angHistory.length;}
...
function onResults(res){
  ...
  const sa=smooth(fa);
  if(lastSmooth!==null&&Math.abs(sa-lastSmooth)<DEAD){q('angleBadge').textContent=Math.round(sa)+'°';return;}
  ...
  if(sa<dT&&!isDown)isDown=true;
  else if(sa>uT&&isDown){addRep();isDown=false;goalAchieved=false;}
  const qual=(!isDown&&sa>uT-10)?'✅ Идеально':(isDown&&sa<dT+20)?'⚠️ Глубоко':'👍 Норма';
  q('qualityBadge').textContent=qual;
  if(currentEx==='squat'&&a2d(lm[11],lm[23],lm[25])<140)tipShow('Спина округляется!','warn');
...
async function startCam(){
  try{
    ...
    const vid=q('video');
    cam=new Camera(vid,{onFrame:async()=>{if(isRunning&&!isPaused&&pose&&vid.readyState>=2)try{await pose.send({image:vid});}catch(e){}},width:640,height:480});
    await cam.start();
  ...
  }catch(e){toast('Ошибка: '+e.message);q('debugLine').textContent='Ошибка: '+e.message;}
}
async function startVid(){
  ...
  try{
    ...
    const p=()=>{if(isRunning&&!isPaused&&!vu.paused&&pose&&vu.readyState>=2)pose.send({image:vu}).catch(()=>{});frameId=requestAnimationFrame(p);};p();
...
function stopAll(){
  ...
  if(pose){try{pose.close();}catch(e){}pose=null;}
  setCtrl(false);
  if(repCount>0){saveSet(true);toast('✅ Тренировка сохранена');}else toast('Стоп');
  stopHiit();
}
function pauseAll(){
...
function startWorkoutTimer(min){
  ...
  const tick=()=>{const m=Math.floor(s/60),sc=s%60;if(d)d.textContent=`${String(m).padStart(2,'0')}:${String(sc).padStart(2,'0')}`;if(s<=10&&s>0)beep(400,.06);if(s<=0){clearInterval(workoutTimerInt);if(Notification.permission==='granted')new Notification('Fitness Pro',{body:'Тренировка окончена!'});speak('Время вышло!');bSuccess();if(d)d.textContent='⏰ Готово!';return;}s--;};
...
function buildEmojiGrid(){
  ...
  el.querySelectorAll('.em-opt').forEach(b=>b.onclick=()=>{avatar=b.dataset.em;avatarIsPhoto=false;renderAvatar();save();el.querySelectorAll('.em-opt').forEach(x=>x.classList.toggle('sel',x===b));});
}
...
function updateLB(){
  ...
  const players=[{name:userName,xp,lvl,avatar,avatarIsPhoto,me:true},...leaderboard].sort((a,b)=>b.xp-a.xp).slice(0,8);
  el.innerHTML=players.map((p,i)=>`<div class="lb-item">
    <div class="lb-rank">${medals[i]||''}</div>
    <div class="lb-av ${p.me ? 'data-me' : ''}">${p.avatarIsPhoto?`<img src="${p.avatar}" alt="">`:p.avatar||'🏆'}</div>
    <div class="lb-name">${p.name}${p.me?' (Вы)':''}</div>
...
function updateSharePreview(){
  ...
  el.textContent=`🏆 Fitness Pro\nУровень ${lvl} · ${xp}/${lvl*100} XP\n💪 Всего повторений: ${totalVolume}\n🔥 Калорий: ${Math.floor(caloriesBurned)}\n⚡ Макс. серия: ${maxStreak}`;
}
function shareResult(){
  const txt=q('sharePreview')?.textContent||'Fitness Pro';
  if(navigator.share)navigator.share({title:'Fitness Pro',text:txt});
...
function buildExGrid(){
  ...
  el.innerHTML=Object.entries(EX).map(([k,v])=>`<div class="ex-card${k===currentEx?' active':''}" data-ex="${k}"><div class="ex-emoji">${v.emoji}</div><div class="ex-name">${v.name}</div><div class="ex-meta">${v.meta}</div></div>`).join('');
...
function speak(txt,urgency=''){
  ...
  const now=Date.now();if(now-lastVoiceTs<2000&&urgency!=='!')return;
  lastVoiceTs=now;speechSynthesis.cancel();
  const u=new SpeechSynthesisUtterance(txt);u.lang='ru-RU';u.volume=.8;speechSynthesis.speak(u);
}
...
window.onload=()=>{
  ...
  load();
  buildExGrid();buildProgsGrid();buildChartTabs();buildEmojiGrid();buildThemeGrid();
  ...
  q('modeVid')?.addEventListener('click',()=>{currentMode='video';q('video').style.display='none';q('vidUp').style.display='';if(q('vidUp').src)q('fileUploadBtn').classList.add('visible');q('modeVid').classList.add('active');q('modeCam').classList.remove('active');toast('🎥 Видео');});
  ...
  q('saveRemBtn')?.addEventListener('click',()=>{toast('⏰ Напоминание сохранено');});
  ...
  q('videoFile')?.addEventListener('change',e=>{
    ...
    q('modeVid').click();toast('🎥 Видео загружено');
  });
