...
// ============================================================
//  DAILY REMINDER — напоминание о тренировке в заданное время
// ============================================================
// Ограничение: это работает только пока страница открыта в браузере (хотя бы в фоне).
// Закрытая вкладка или выключенный экран не "разбудят" сайт — для этого нужен push
// с сервера, которого здесь нет. В рамках этого ограничения логика полностью рабочая.
let reminderInterval=null;
let lastReminderFiredKey=null; // 'YYYY-MM-DDTHH:MM' последнего срабатывания — защита от повторного показа в ту же минуту

function loadReminderSettings(){
  const r=JSON.parse(localStorage.getItem('fp_reminder')||'{}');
  const enabled=r.enabled||false;
  const hour=r.hour!==undefined?r.hour:19;
  const minute=r.minute!==undefined?r.minute:0;
  const dc=q('dailyCheck');if(dc)dc.checked=enabled;
  const rh=q('remH');if(rh)rh.value=hour;
  const rm=q('remM');if(rm)rm.value=minute;
  updateNotifStatus(enabled,hour,minute);
  if(enabled)startReminderWatch();
}
function saveReminderSettings(){
  const enabled=q('dailyCheck')?.checked||false;
  const hour=Math.min(23,Math.max(0,parseInt(q('remH')?.value)||0));
  const minute=Math.min(59,Math.max(0,parseInt(q('remM')?.value)||0));
  localStorage.setItem('fp_reminder',JSON.stringify({enabled,hour,minute}));
  updateNotifStatus(enabled,hour,minute);
  if(enabled)startReminderWatch();else stopReminderWatch();
  return{enabled,hour,minute};
}
function updateNotifStatus(enabled,hour,minute){
  const el=q('notifStatus');if(!el)return;
  if(!enabled){el.textContent='Напоминание выключено';el.classList.remove('active');return;}
  const hh=String(hour).padStart(2,'0'),mm=String(minute).padStart(2,'0');
  if(Notification.permission!=='granted'){
    el.textContent=`⚠️ Время ${hh}:${mm} сохранено, но уведомления не разрешены — нажмите «Разрешить уведомления» выше`;
    el.classList.remove('active');
  }else{
    el.textContent=`✅ Активно: напоминание каждый день в ${hh}:${mm} (пока сайт открыт)`;
    el.classList.add('active');
  }
}
function startReminderWatch(){
  if(reminderInterval)return; // уже запущено
  checkReminderTick(); // сразу одна проверка, не дожидаясь первой минуты интервала
  reminderInterval=setInterval(checkReminderTick,30000); // проверяем дважды в минуту — надёжнее, чем раз в 60с
}
...
function checkReminderTick(){
  ...
  const fireKey=now.toISOString().slice(0,16); // YYYY-MM-DDTHH:MM — уникально на минуту
  if(fireKey===lastReminderFiredKey)return; // уже сработало в эту минуту
  lastReminderFiredKey=fireKey;
  if(Notification.permission==='granted'){
    try{new Notification('FitPulse',{body:'Пора тренироваться! 💪',icon:'./icon-192.png'});}catch(e){}
  }
  speak('Пора тренироваться!','!');
  toast('💪 Время тренировки!',4000);
}
...
window.onload=()=>{
  ...
  load();loadVoicePrefs();loadReminderSettings();
  ...
  q('notifBtn')?.addEventListener('click',()=>Notification.requestPermission().then(p=>{
    toast(p==='granted'?'✅ Разрешено':'❌ Отклонено');
    const r=JSON.parse(localStorage.getItem('fp_reminder')||'{}');
    updateNotifStatus(r.enabled||false,r.hour!==undefined?r.hour:19,r.minute!==undefined?r.minute:0);
  }));
  q('publishNowBtn')?.addEventListener('click',async()=>{await publishToCloud(false);await updateLB();});
  q('saveRemBtn')?.addEventListener('click',()=>{
    const{enabled,hour,minute}=saveReminderSettings();
    toast(enabled?`⏰ Сохранено: напоминание в ${String(hour).padStart(2,'0')}:${String(minute).padStart(2,'0')}`:'⏰ Напоминание выключено');
  });
  q('dailyCheck')?.addEventListener('change',()=>{saveReminderSettings();});
